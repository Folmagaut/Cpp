...
public:
    ...
    char GetPixel(Point p) const noexcept {
        ...
        Point localCoords = {p.x % Tile::SIZE, p.y % Tile::SIZE};
        return tiles_[tileIndex]->GetPixel(localCoords);
    }
    ...
    void SetPixel(Point p, char color) {
        ...
        int tileIndex = GetTileIndex(p);
        Point localCoords = {p.x % Tile::SIZE, p.y % Tile::SIZE};
        tiles_[tileIndex].Write([&](Tile& tile) {
        tile.SetPixel(localCoords, color);
        });
    }
