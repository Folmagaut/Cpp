...
// если надо, то поменяю на библиотеку из теории (из прекода),
// но оба варианта библиотек рабочие и проходят проверки в тренажёре
#include "json.h"
...
void PrintValue(const Array& array, const PrintContext& context) {
    context.out << "["sv;
    bool first = true;
...
void PrintValue(const Dict& dict, const PrintContext& context) {
    context.out << "{ "sv;
    bool first = true;
