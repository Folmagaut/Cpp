...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text[0] == '-' && text[1] == '-') {
            is_broken = true;
            return {text, is_minus, IsStopWord(text), is_broken};
        }
...
int main() {
    ...
    (void) search_server.AddDocument(1, "пушистый на кот пушистый-хвост"s, DocumentStatus::ACTUAL, {7, 2, 7});
    (void) search_server.AddDocument(2, "и в на пёс и птица"s, DocumentStatus::ACTUAL, {1, 4, 1});
    ...
    if (search_server.FindTopDocuments("птица пушистый-хвост"s, documents)) {
