...
public:
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        MakeSafeCopy(values.begin(), values.end());
    }

    SingleLinkedList(const SingleLinkedList& other) {
        MakeSafeCopy(other.begin(), other.end());
    }
...
private:
    ...
    void MakeSafeCopy(InputIt it_begin, InputIt it_end) {
       
        SingleLinkedList sll_tmp;
        auto head_ptr = &sll_tmp.head_;

        for (auto sll_tmp_it = it_begin; sll_tmp_it != it_end; ++sll_tmp_it) {
            auto new_node = new Node(*sll_tmp_it, nullptr);
...
// Исправлено, хотя, приведённая до этого функция была как раз скопирована с equal и выполняла то же самое.
// Для чего нужно подцеплять целую библиотеку algorithm если оттуда можно использовать сырой код двух алгоритмов?
template <typename Type>
